          ...
          <div v-for="course in courses" :key="course.id" class="col-lg-3 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
            <div class="course-item shadow d-flex flex-column h-100">
              ...
              <div class="d-flex">
                ...
                <small class="flex-fill text-center py-1 px-2">
                  <i class="fa fa-user me-2"></i>{{ course.level === 'Beginner' ? 'Người mới' : course.level === 'Intermediate' ? 'Trung cấp' : course.level === 'Advanced' ? 'Nâng cao' : course.level || 'Người mới' }}
                </small>
              </div>
              <div class="d-flex">
                ...
                <small class="py-1 px-2 fw-bold fs-6 text-center" v-if="!course.isFree">₹ {{ course.price || 0 }}</small>
